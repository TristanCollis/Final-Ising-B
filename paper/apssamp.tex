...
In this section, we provide an overview of the Ising model,a fundamental concept in statistical mechanics used to describe the behavior of magnetic systems.

For this project, we concentrate on the 2D Ising model, which comprises a lattice of spins ${+1, -1}$ and is commonly employed to approximate ferromagnetic materials. In this model, the energy function is typically defined such that aligned neighbors possess lower energy compared to opposing neighbors.

One notable property stemming from this energy definition is that, due to spins tending to align with neighbors of the same orientation to minimize energy, the lattice tends to align with an external magnetic field at low temperatures. This behavior is a consequence of spins converging towards the lowest energy state, leading to an alignment with the external magnetic field.
