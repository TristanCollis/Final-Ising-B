...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{burin,randomLattice.png}
    \caption{Phase Diagram}
    \label{fig:enter-label}
\end{figure}
...
When plotting magnetization vs temperature for constant magnetic field (figure \ref{fig:1dscans,fixedB}), we see extreme results at low temperature, before results converge at high temperature when there is high disorder.  

Plotting magnetization vs external field (figure \ref{fig:1dscans,fixedT}), we see a phase transition occurring at T=0 that gets smoothed out as T rises.
