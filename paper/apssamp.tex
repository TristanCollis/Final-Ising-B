...
For this project, we focus on the 2D Ising model which consists of a lattice of spins $\{+1, -1\}$, and is often used to approximate ferromagnetic materials. 
We typically define the energy function such that aligned neighbors are in a lower energy state than opposing neighbors. 
A property that arises from this definition of energy is as a result of spins convering towards the lowest energy state, the lattice tends to converge towards the external magnetic field for low temeraptures.
